...
void setup()
{
  ...
  // Create/Open file 
  myFile = SD.open("straj.csv", FILE_WRITE);
  delay(1000);
...
void loop()
{
  ...
  if (running) {
    if ((current_time - start_time) < duration) {
      ...
      float d = max_steering_delta * sin((2 * M_PI * (current_time - start_time))/duration);
      float R = l / tan(d * M_PI / 180);
      float n = (R < 0) ? sqrt((-R - w/2) / (-R + w/2)) : sqrt((R + w/2)/(R - w/2));
